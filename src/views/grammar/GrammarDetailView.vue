...
<template>
  ...
  <div v-else-if="lesson" class="p-4 md:p-8 max-w-4xl mx-auto mb-20 md:mb-0">
    ...
    <header class="mb-8">
      <div class="flex items-center gap-2 mb-2">
        <span
          class="bg-gray-100 text-gray-600 text-xs font-bold px-2 py-1 rounded-md uppercase tracking-wide"
        >
          {{ lesson.category }}
        </span>
    ...
    <div class="card p-6 md:p-10 mb-10 space-y-6">
      <template v-for="(block, index) in parsedContent" :key="index">
        <p
          ...
          v-html="block.text"
          class="text-gray-700 text-lg leading-relaxed font-body"
        ></p>
    ...
    <div v-if="lesson.examples.length > 0">
      ...
      <div class="grid gap-4 md:grid-cols-1">
        <div
          v-for="(ex, index) in lesson.examples"
          :key="index"
        ...
        >
          ...
          <button
            @click="playGrammarAudio(lesson.id, index, ex.japanese)"
  ...
  <div v-else class="flex flex-col items-center justify-center p-12 text-center h-[60vh]">
    ...
    <button @click="router.push('/grammar')" class="btn-3d btn-primary">Retour à la liste</button>
  </div>
</template>
