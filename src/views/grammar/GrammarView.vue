...
<template>
  <div class="flex flex-col items-center w-full px-4 mb-4 md:mb-0">
    ...
    <div v-if="isLoading" class="w-full flex justify-center py-24">
      <LoadingSpinner size="xl" text="Préparation des leçons..." />
    </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 w-full max-w-6xl px-0 md:px-4">
        ...
        >
          <button
            v-if="authStore.user"
            @click.stop="userStore.toggleMastery(lesson.id)"
          ...
          <h3
            class="text-sm md:text-2xl font-bold transition-colors mb-1 md:mb-2 mt-8 md:mt-6"
            :class="
              isMastered(lesson.id)
                ? 'text-tanuki-green'
          ...
          <p
            class="text-gray-500 text-xs md:text-base mb-3 md:mb-6 line-clamp-2 md:line-clamp-3 grow"
          >
            {{ lesson.summary }}
          </p>
          ...
          <div class="flex items-center text-tanuki-green font-bold text-xs md:text-base mt-auto">
            ...
            <ChevronRight
              class="w-4 h-4 ml-1 md:w-5 md:h-5 group-hover:translate-x-1 transition-transform"
            />
          </div>
        </div>
      </div>
