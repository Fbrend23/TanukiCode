...
import { ArrowLeft, RefreshCw, Check, Volume2, HelpCircle, Trophy, Hammer, Heart, Flame } from 'lucide-vue-next';
...
<template>
    <div class="flex flex-col items-center w-full px-4 font-outfit min-h-screen">
        ...
        <div class="w-full flex flex-col items-center max-w-3xl mx-auto">
            ...
            <!-- Score & Streak -->
            <div class="relative w-full max-w-lg mx-auto z-10 mb-6">
                <div
            ...
            <div v-if="currentSentence" class="w-full flex flex-col items-center">
                ...
                <!-- Main Game Card -->
                <div
                    class="card w-full text-center mb-8 relative overflow-hidden group p-6 md:p-10 min-h-[30vh] flex flex-col items-center justify-center bg-white border-2 border-tanuki-green shadow-lg">
                    ...
                    <div
                        ...
                        <Hammer class="w-3 h-3" />
                        <span>Phrase</span>
                    </div>

                    <!-- Context -->
                    <div class="mb-8">
                        <h2 class="text-xl md:text-3xl font-bold text-tanuki-brown-dark leading-relaxed">
                    ...
                    <div class="w-full min-h-[100px] border-b-2 border-tanuki-brown/20 mb-8 flex flex-wrap gap-2 justify-center items-center pb-4 transition-colors"
                        ...
                        <div v-for="(token, idx) in answerTokens" :key="token.id" draggable="true"
                            ...
                            class="animate-pop-in btn-3d btn-primary py-2 px-3 shadow-md text-sm md:text-base cursor-move flex items-center gap-2 select-none active:cursor-grabbing hover:scale-105 transition-transform"
                    ...
                    <div class="mb-4 transition-opacity duration-300 min-h-[1.5rem]"
                        :class="(hasChecked && !isCorrect) ? 'opacity-100' : 'opacity-0 select-none pointer-events-none'">
                        <span class="text-red-500 font-bold animate-bounce-short inline-block">
                    ...
                    <div v-if="isCorrect"
                        ...
                        <div class="text-green-600 flex flex-col items-center animate-bounce-short mb-4">
                ...
                <div class="w-full max-w-2xl mb-8 p-6 border-2 border-tanuki-green rounded-3xl bg-white/50 relative">
                    <div
                    ...
                    <div class="flex flex-wrap justify-center gap-3">
                        <button v-for="token in poolTokens" :key="token.id" @click="addToAnswer(token)"
                            :disabled="token.used"
                            class="btn-3d btn-secondary py-3 px-4 text-sm md:text-base h-auto whitespace-normal transition-all duration-300"
                ...
                <div class="flex gap-4 w-full max-w-md justify-center items-center">
                    <button v-if="!isCorrect" @click="showHint = !showHint"
                    ...
                    <button v-if="!isCorrect && answerTokens.length > 0" @click="checkAnswer"
                        class="btn-3d btn-primary px-8 animate-pop-in">
                        <span>Valider</span>
                    </button>

                    <button v-if="isCorrect" @click="initRound"
                        class="btn-3d btn-gold flex items-center gap-2 px-8 animate-pop-in">
