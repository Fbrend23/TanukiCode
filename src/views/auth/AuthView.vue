      ...
      <form @submit.prevent="handleAuth" class="space-y-4">
        ...
        <div v-if="!isResetMode">
          <div class="flex justify-between items-center mb-1">
            <label for="password" class="block text-sm font-bold text-tanuki-brown"
              >Mot de passe</label
            >
            <button
              v-if="isLogin"
        ...
        >
          <component
            :is="authMessage.type === 'error' ? AlertCircle : CheckCircle"
            class="w-4 h-4"
          />
          {{ authMessage.text }}
        </div>
        ...
        <p v-if="!isLogin" class="text-[10px] text-center text-gray-400 mt-4 leading-tight">
          ...
          <RouterLink to="/legal" class="underline hover:text-tanuki-green"
