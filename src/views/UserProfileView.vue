...
import AttendanceCalendar from '@/components/gamification/AttendanceCalendar.vue'
import MasteryBadges from '@/components/gamification/MasteryBadges.vue'
...
const getColorClass = (colorName: string) => {
    // If it's a file path (image), return white bg
    ...
    try {
        ...
        if (data) {
            avatars.value = data
                ...
                .map(file => ({
                    name: file.name.replace('tanuki_', '').replace('.png', '').replace(/_/g, ' '),
                    value: file.name,
                    src: supabase.storage.from('avatars').getPublicUrl(file.name).data.publicUrl
                }))
        }
    } catch (e) {
        console.error('Unexpected error loading avatars:', e)
    }
}
...
const getAvatarSrc = (avatarValue: string) => {
    ...
    // Directly construct the URL for the filename
    // We assume any other value is a valid filename from storage or legacy
...
const startEditing = () => {
    ...
    formImage.value = userStore.avatarImage || 'default'
...
const saveProfile = async () => {
    ...
    try {
        await userStore.updateProfile(formUsername.value, formColor.value, formImage.value)
...
<template>
    <div class="flex flex-col items-center w-full px-4 min-h-200">
        <!-- Loading State -->
        <div v-if="isLoading" class="w-full flex justify-center py-32">
            <LoadingSpinner size="xl" text="Chargement du profil..." />
        </div>

        <div v-else class="w-full max-w-2xl flex flex-col items-center">
            <div class="text-center mb-4 md:mb-8">
                <h1 class="text-3xl md:text-4xl font-display font-bold text-tanuki-green">Mon Profil</h1>
            </div>
            ...
            <div class="card flex p-1 mb-6 shadow-none border-2 border-tanuki-green w-full max-w-md bg-white">
                <button @click="activeTab = 'profile'"
                    :class="['flex-1 py-2 rounded-2xl font-bold transition-all text-sm flex items-center justify-center gap-2', activeTab === 'profile' ? 'bg-tanuki-green text-white shadow-sm' : 'text-gray-500 hover:text-tanuki-green']">
                    <User class="w-4 h-4" /> Profil
                </button>
                <button @click="activeTab = 'stats'"
                    :class="['flex-1 py-2 rounded-2xl font-bold transition-all text-sm flex items-center justify-center gap-2', activeTab === 'stats' ? 'bg-tanuki-green text-white shadow-sm' : 'text-gray-500 hover:text-tanuki-green']">
                    ...
                    <div class="card p-4 lg:p-8 relative">
                        ...
                        <div class="flex flex-col items-center gap-6 w-full">
                            ...
                            <div class="relative group">
                                <div class="w-24 h-24 rounded-full flex items-center justify-center text-white shadow-inner transition-colors border-4 border-white ring-4 ring-tanuki-green/20 overflow-hidden"
                                    :class="isEditing ? getColorClass(formColor) : getColorClass(userStore.avatarColor)">
                                    <!-- Image Avatar -->
                                    <img v-if="(isEditing ? formImage : userStore.avatarImage) !== 'default'"
                                        :src="getAvatarSrc(isEditing ? formImage : (userStore.avatarImage || 'default')) || ''"
                                        alt="Avatar" class="w-full h-full object-cover p-1" />
                                    <!-- Default Tanuki -->
                                    <img v-else :src="defaultTanuki" alt="Tanuki"
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-4 sm:grid-cols-6 gap-2 max-w-md mx-auto">
                                        ...
                                        <button @click="formImage = 'default'"
                                            ...
                                            :class="[formImage === 'default' ? 'border-tanuki-brown ring-2 ring-offset-2 ring-tanuki-brown/20' : 'border-transparent']"
                                            title="Tanuki Basic">
                                            <img :src="defaultTanuki" alt="Default"
                                                class="w-full h-full object-contain" />
                                        </button>
                                        <!-- Custom Avatars -->
                                        <button v-for="avatar in avatars" :key="avatar.value"
                                            @click="formImage = avatar.value"
                                            class="aspect-square rounded-xl border-2 transition-all transform hover:scale-105 overflow-hidden relative p-0.5 bg-white"
                                            :class="[formImage === avatar.value ? 'border-tanuki-brown ring-2 ring-offset-2 ring-tanuki-brown/20' : 'border-transparent']"
                                            :title="avatar.name">
                    ...
                    <div class="card p-8 space-y-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div v-else
                                ...
                                <h4 class="font-bold text-tanuki-green text-sm uppercase tracking-wide mb-2">Nouveau mot
                                    de
                                    passe</h4>
                                <div>
