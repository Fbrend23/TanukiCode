...
<template>
  <div class="flex flex-col items-center justify-start min-h-[60vh] pb-6 md:pb-0">
    ...
    <div v-else class="w-full flex flex-col items-center">
      <h2 class="text-3xl md:text-4xl font-display font-bold text-tanuki-green mb-1 md:mb-8">
        Flashcards
      </h2>
      ...
      <div class="card flex flex-wrap justify-center p-1 mb-2 md:mb-4 max-w-full">
        <button
          @click="mode = 'hiragana'"
          :class="['btn-pill', mode === 'hiragana' ? 'btn-pill-active' : 'btn-pill-inactive']"
        >
          Hiragana
        </button>
        <button
          @click="mode = 'katakana'"
          :class="['btn-pill', mode === 'katakana' ? 'btn-pill-active' : 'btn-pill-inactive']"
        >
          Katakana
        </button>
        <button
          @click="mode = 'kanji'"
          :class="['btn-pill', mode === 'kanji' ? 'btn-pill-active' : 'btn-pill-inactive']"
        >
          Kanji
        </button>
        <button
          @click="mode = 'vocabulary'"
          :class="['btn-pill', mode === 'vocabulary' ? 'btn-pill-active' : 'btn-pill-inactive']"
        >
          Vocabulaire
        </button>
        <button
          @click="mode = 'sentences'"
          :class="['btn-pill', mode === 'sentences' ? 'btn-pill-active' : 'btn-pill-inactive']"
        >
          Phrases
        </button>
      </div>
      ...
      <!-- Flashcard Scene -->
      <div
        class="scene w-72 h-80 md:w-xl md:h-104 perspective-1000 cursor-pointer group"
        @click="flipCard"
      >
        <div
        ...
        >
          ...
          >
            <div class="text-center px-4">
              <span
                :class="[
                  'block font-bold text-tanuki-brown-dark mb-2 break-word transition-all',
                  fontSizeClass,
                ]"
                >{{ frontText }}</span
              >
          ...
          >
            ...
            <span v-if="mode !== 'kanji'" class="text-xl opacity-80">{{
              'translation' in currentCard || currentCard.meaning ? 'Signification' : 'Romaji'
            }}</span>
            <span
              v-if="
                'translation' in currentCard ||
                (currentCard.meaning && !Array.isArray(currentCard.meaning))
              "
              class="text-sm mt-2 opacity-60"
              >({{ currentCard.romaji }})</span
            >
          </div>
        </div>
      </div>
      ...
      <div class="mt-6 flex gap-4">
        <button
          @click="nextCard"
          class="btn-3d btn-gold flex items-center gap-2 py-3 px-8 text-lg w-auto"
        >
