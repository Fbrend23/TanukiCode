...
const filteredVocabulary = computed(() => {
  ...
  return vocabulary.filter((word) => {
    if (selectedCategories.value.length > 0 && !selectedCategories.value.includes(word.category))
      return false
...
const filteredSentences = computed(() => {
  ...
  return sentences.filter((s) => {
    // Category Filter
    if (
      selectedSentenceCategories.value.length > 0 &&
      !selectedSentenceCategories.value.includes(s.category)
    )
      return false
  ...
  <div class="flex flex-col items-center w-full px-4 py-0 pb-24 font-outfit">
    ...
    <template v-else>
      <div class="flex flex-col items-center w-full max-w-6xl mb-6">
        <div class="text-center mb-6">
          ...
          <div class="card flex p-1 shadow-none border-2 border-tanuki-green bg-white inline-flex cursor-default">
            <button @click="activeTab = 'words'"
              class="px-6 py-2 rounded-full font-bold transition-all flex items-center gap-2" :class="activeTab === 'words'
                ? 'bg-tanuki-green text-white shadow-sm'
                : 'text-stone-500 hover:text-tanuki-green'
                ">
              Mots
            </button>
            <button @click="activeTab = 'sentences'"
              class="px-6 py-2 rounded-full font-bold transition-all flex items-center gap-2" :class="activeTab === 'sentences'
                ? 'bg-tanuki-green text-white shadow-sm'
                : 'text-stone-500 hover:text-tanuki-green'
                ">
              Phrases
        ...
        <div v-if="activeTab === 'words'"
          ...
          <MasteryBar v-if="authStore.user" label="Progression Vocabulaire" :current="masteredWords"
            :total="totalWords" />
          <GuestCard v-else />
            ...
            <div v-for="word in filteredVocabulary" :key="word.romaji" @click="playWordAudio(word)"
              ...
              <div class="mb-1">
                <p class="text-xs text-tanuki-brown/60 font-bold tracking-wide uppercase">
                  {{ categoryTranslations[word.category] }}
                </p>
              </div>

              <div class="flex flex-col items-center justify-center grow py-2">
                <h2 class="text-2xl md:text-3xl font-bold text-tanuki-brown mb-1">
                  {{ word.word }}
                </h2>
        ...
        <div v-if="activeTab === 'sentences'"
          ...
          <!-- Progress Bar or Guest Message -->
          <MasteryBar v-if="authStore.user" label="Progression Phrases" :current="masteredSentences"
            :total="totalSentences" class="mb-6 max-w-2xl mx-auto" />
          <GuestCard v-else />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div v-for="s in filteredSentences" :key="s.id" @click="playSentence(s)"
                ...
                :class="playingId === s.id
                  ? 'bg-tanuki-green/5 border-tanuki-green'
                  : 'hover:border-tanuki-green-light'
                  ">
                ...
                <div class="grow flex flex-col justify-center py-2">
                  <p class="text-lg font-bold text-tanuki-brown-dark leading-relaxed mb-1">
                    {{ s.japanese }}
                  </p>
    ...
    <FilterModal v-model:isOpen="isFilterModalOpen">
      <div class="flex flex-col gap-3">
        ...
        <div class="flex flex-wrap gap-2">
          <button v-for="cat in vocabCategories" :key="cat" @click="toggleCategory(cat)" :class="[
    ...
    <FilterModal v-model:isOpen="isSentenceFilterModalOpen">
      <div class="flex flex-col gap-3">
        <h3 class="font-bold text-tanuki-brown">Catégorie</h3>
        <div class="flex flex-wrap gap-2">
          <button v-for="cat in sentenceCategories" :key="cat" @click="toggleSentenceCategory(cat as string)" :class="[
            'px-3 py-1.5 rounded-lg text-sm font-bold whitespace-nowrap transition-colors border-2 flex items-center gap-2',
            (
              cat === 'all'
                ? selectedSentenceCategories.length === 0
                : selectedSentenceCategories.includes(cat as string)
            )
              ? 'bg-tanuki-green text-white border-tanuki-green'
              : 'bg-white text-gray-500 border-gray-200 hover:border-tanuki-green/50',
          ]">
