  ...
  try {
    ...
    writer.value = HanziWriter.create(writerContainer.value, props.character, {
      ...
      charDataLoader: (char, onComplete) => {
        fetch(
          `https://raw.githubusercontent.com/chanind/hanzi-writer-data-jp/master/data/${char}.json`,
        )
          .then((res) => {
...
<template>
  <div class="flex flex-col items-center">
    <!-- Canvas Container -->
    <div
      class="relative bg-white rounded-xl shadow-inner border-2 border-tanuki-beige p-4 mb-4 min-h-55 flex items-center justify-center overflow-hidden"
    ...
    >
      ...
      >
        ...
        <span class="text-tanuki-brown/70 font-bold text-sm"
          >Désolé, je ne connais pas encore le tracé de ce Kanji !</span
        >
      </div>
      ...
      <button
        ...
        :class="
          mode === 'view'
            ...
            : 'bg-white text-tanuki-green border border-tanuki-green hover:bg-green-50'
        "
      >
        <Play class="w-4 h-4" />
        Voir
    ...
    <p
      v-if="!initialMode || mode === 'quiz'"
      class="text-xs text-stone-500 italic max-w-xs text-center mt-2"
    >
      <span v-if="mode === 'view'">Regardez l'animation pour apprendre l'ordre des traits.</span>
      <span v-else
        >Dessinez le caractère par-dessus le modèle.
