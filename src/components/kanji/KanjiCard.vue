...
<template>
  ...
  >
    ...
    <div
      class="absolute top-2 right-2 font-bold px-2 py-0.5 rounded-full text-[10px]"
      :class="
        isMastered ? 'text-tanuki-green bg-tanuki-green/10' : 'text-tanuki-brown bg-tanuki-brown/10'
      "
    >
      N{{ kanji.jlpt }}
    </div>
    ...
    <div
      class="text-6xl font-display font-medium mb-4 transition-colors"
      :class="
        isMastered ? 'text-tanuki-green' : 'text-tanuki-brown-dark group-hover:text-tanuki-green'
      "
    >
      {{ kanji.character }}
    </div>
    ...
    <div class="text-center mb-4 min-h-[1.5em]">
      <p class="font-bold text-tanuki-brown text-lg leading-tight">
    ...
    <div class="w-full space-y-2 border-t border-tanuki-beige/30 pt-3">
      <div class="flex justify-between items-baseline">
        ...
        <span class="text-base font-bold text-blue-900 font-body">{{
          kanji.onyomi.slice(0, 2).join('・')
        }}</span>
      </div>
      <div class="flex justify-between items-baseline">
        <span class="text-xs text-tanuki-green font-bold tracking-widest">KUN</span>
        <span class="text-base font-bold text-tanuki-brown-dark font-body">{{
