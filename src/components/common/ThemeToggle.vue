...
<template>
  <button
    ...
    :title="isDark ? 'Passer en mode jour' : 'Passer en mode nuit (Night Tanuki)'"
    aria-label="Basculer le thème"
  >
    <div class="relative w-6 h-6">
      <Sun
        class="w-6 h-6 text-tanuki-gold absolute inset-0 transition-all duration-500 transform"
        :class="isDark ? 'rotate-90 opacity-0 scale-50' : 'rotate-0 opacity-100 scale-100'"
      />
      <Moon
        class="w-6 h-6 text-tanuki-beige absolute inset-0 transition-all duration-500 transform"
